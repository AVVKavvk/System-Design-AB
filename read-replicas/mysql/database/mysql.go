...
func CheckDatabaseConnection(db *gorm.DB) {
	...
	// 2. Check WRITE Connection (Should be Primary ID: 1)
	db.Clauses(dbresolver.Write).Raw("SELECT @@server_id").Scan(&serverID)
	log.Printf("WRITE Query handled by Server ID: %s", serverID)
}
